...
const getValidAuthorizerKeysMock = getValidAuthorizerKeys as jest.MockedFunction<typeof getValidAuthorizerKeys>;
...
describe("basicAuthHandler", () => {
  ...
  ])("returns isAuthorized==false when the body is null, doesn't contain 'email', or the email is empty", async (body) => {
    const response = await basicAuthHandler({
      headers: { Authorization: `Basic ${GOOD_KEY}` },
      body,
    });
  ...
  it.each([[{}], [{ cookies: "yumYumYum" }]])("returns isAuthorized==false when the headers are empty or don't contain 'Authorization'", async (headers) => {
    const response = await basicAuthHandler({
      headers,
      body: (new URLSearchParams({ email: "test@example.com" })).toString(),
    });
